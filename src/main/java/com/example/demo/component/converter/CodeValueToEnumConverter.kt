...
class CodeValueToEnumConverter<T : ICodeValue>(private val enumType: Class<T>) : Converter<Int, T> {
    //    private val idEnumMap: MutableMap<String, T> = hashMapOf()
    private val codeEnumMap: MutableMap<Int, T> = hashMapOf()
    private val logger = LoggerFactory.getLogger(CodeValueToEnumConverter::class.java)
...
class CodeValueToEnumConverterFactory : ConverterFactory<Int, ICodeValue> {
    private val logger = LoggerFactory.getLogger(CodeValueToEnumConverterFactory::class.java)

    override fun <T : ICodeValue> getConverter(targetType: Class<T>): Converter<Int, T> {
//        var converter: Converter<String, T> = CONVERTERS[targetType]
        ...
        if (converter == null) {
            val cc = CodeValueToEnumConverter(targetType)
            converter = CodeValueToEnumConverter(targetType)
